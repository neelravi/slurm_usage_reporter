...
class Report:
    ...
    def summary_report(self, title):
        ...
        print(title)
        print("Report generated on                    {}".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M")))
        print("Data gathered between           {} - {}".format(self.data.start_date, self.data.end_date))
        print("-" * 55)
        print("Jobs submitted:                      {:6d}  ({:6.2f} %)".format( self.data.total_entries,        float(self.data.total_entries)        / self.data.total_entries * 100 ))
        print("Jobs executed successfully:          {:6d}  ({:6.2f} %)".format( self.data.total_completed,      float(self.data.total_completed)      / self.data.total_entries * 100 ))
        print("Jobs executed but timed out:         {:6d}  ({:6.2f} %)".format( self.data.total_timeout,        float(self.data.total_timeout)        / self.data.total_entries * 100 ))
        print("Jobs executed but failed:            {:6d}  ({:6.2f} %)".format( self.data.total_failed,         float(self.data.total_failed)         / self.data.total_entries * 100 ))
        print("Jobs where the node failed:          {:6d}  ({:6.2f} %)".format( self.data.total_node_fail,      float(self.data.total_node_fail)      / self.data.total_entries * 100 ))
        print("Jobs cancelled automatically:        {:6d}  ({:6.2f} %)".format( self.data.total_cancelled_auto, float(self.data.total_cancelled_auto) / self.data.total_entries * 100 ))
        print("Jobs cancelled by user:              {:6d}  ({:6.2f} %)".format( self.data.total_cancelled_user, float(self.data.total_cancelled_user) / self.data.total_entries * 100 ))
        print("Jobs still pending:                  {:6d}  ({:6.2f} %)".format( self.data.total_pending,        float(self.data.total_pending)        / self.data.total_entries * 100 ))
        print("Jobs requeued:                       {:6d}  ({:6.2f} %)".format( self.data.total_requeued,       float(self.data.total_requeued)       / self.data.total_entries * 100 ))
        print("Jobs still running:                  {:6d}  ({:6.2f} %)".format( self.data.total_running,        float(self.data.total_running)        / self.data.total_entries * 100 ))
    ...
    def user_consumption_report(self):
        ...
        print("{:10s}   {:19s}   {:20s}".format("Username", "Jobs", "Cpu_hours"))
        print("-" * 55)
        for key in sorted(self.data.users.keys()):
            jobs = self.data.users[key].total_jobs()
            cpuh = self.data.users[key].total_cpuh()
            print("{:10s}   {:8d} ({:6.2f} %)   {:9.2f} ({:6.2f} %)".format(key, jobs, 100.0 * jobs / self.data.total_entries , cpuh, 100.0 * cpuh / self.data.total_compute_hours))
        print("-" * 55)
        print("{:10s}   {:8d}              {:9.2f} ({:6.2f} %)".format("Total", self.data.total_entries, self.data.total_compute_hours, 100.0 * self.data.total_compute_hours / self.data.total_avail_cpuh))
    ...
    def histogram(self, title, header, bins, data):
        ...
        print(title)
        print("{:15} | {:6} | {:8} - {:8}".format(header, "amount", "percent", "cumulat"))
        print("-" * 46)
        cum = 0
        for i in range(0, len(values)):
            percent = 100.0 * values[i] / self.data.total_completed
            cum = 100.0  * sum(values[:i + 1]) / self.data.total_completed
            print("{:6d} - {:6d} | {:6d} | {:6.2f} % - {:6.2f} %".format(limits[i], limits[i+1], values[i], percent, cum))
...
class Slurm_Reports(object):
    ...
    def valid_date_string(self, string):
        ...
        if not self.pattern_date_format.match(string):
            msg = "{} is not a valid date string. Format is YYYY-MM-DD".format(string)
    ...
    def main(self):
        ...
        try:
            ...
            total_avail_cpuh = int(self.config.get("General", "avail_cpu_number")) * \
                            int(timedelta.total_seconds() / 3600)

            data = Data(total_avail_cpuh, self.args.start, self.args.end)
            self.sacct_command[0] = self.config.get("General", "sacct_path")
            for line in subprocess.run(self.sacct_command, stdout=subprocess.PIPE, check=True, universal_newlines=True).stdout.split("\n"):
                if line:
            ...
            print("{} returned code {}".format(e.cmd, e.returncode))
            print("Message is {}".format(e.output))
